...
module {
  ...
  public func accountIdentifierToTextDefaultSub (_principal: Principal) : T.AccountIdentifierToTextResult {
        let blobResult = getAccountIdAsBlob(_principal);
        switch(blobResult){
          case(#ok(blob)){
            #ok(Hex.encode(Blob.toArray(blob)));
          };
  ...
  /**
    ...
    * Returns : Principal
    */
  public func generateInvoiceSubaccount (args : GenerateInvoiceSubaccountArgs) : [Nat8] {
    ...
    Array.append(crc32Bytes, hashSum);
  };
  ...
  public func getDefaultAccount(args : DefaultAccountArgs) : [Nat8] {
  ...
  public func getICPAccountIdentifier(args : GetICPAccountIdentifierArgs) : [Nat8] {
    A.accountIdentifier(args.principal, args.subaccount);
  };
   public func getAccountIdAsBlob(_principal:Principal) :T.AccountIdentifierToBlobResult{
                var subAcc=A.defaultSubaccount();      
                var identifier=A.accountIdentifier(_principal,subAcc);
		   let err = {
      kind = #InvalidAccountIdentifier;
      message = ?"Invalid account identifier";
    };
